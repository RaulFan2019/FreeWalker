...
public class CircleProgress extends View {
    ...
    private void init(Context context, AttributeSet attrs) {
        mContext = context;
        mDefaultSize = DensityU.dip2px(mContext, DEFAULT_SIZE);
    ...
    private void initAttrs(AttributeSet attrs) {
        ...
        mPrecisionFormat = DensityU.getPrecisionFormat(mPrecision);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        setMeasuredDimension(DensityU.measure(widthMeasureSpec, mDefaultSize),
                DensityU.measure(heightMeasureSpec, mDefaultSize));
    }
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        updateArcPaint();
//        YLog.e(TAG, "onSizeChanged: 控件大小 = " + "(" + w + ", " + h + ")"
...
//                + ";圆的外接矩形 = " + mRectF.toString());
    ...
    private float getBaselineOffsetFromY(Paint paint) {
        return DensityU.measureTextHeight(paint) / 2;
    }
    ...
    private void drawText(Canvas canvas) {
        // 计算文字宽度，由于Paint已设置为居中绘制，故此处不需要重新计算
         float textWidth = mValuePaint.measureText(String.valueOf(mValue));
//         float x = mCenterPoint.x - textWidth / 2;
        ...
        if (mUnit != null) {
            canvas.drawText(mUnit.toString(), mCenterPoint.x + DensityU.dip2px(mContext,14), mValueOffset, mUnitPaint);
        }
    }
...
//        mArcPaint.setShader(mSweepGradient);
        mArcPaint.setColor(mValueColor);
    }
...
//                YLog.e(TAG, "onAnimationUpdate: percent = " + mPercent
...
//                        + ";value = " + mValue);
    ...
    public void setPrecision(int precision) {
        ...
        mPrecisionFormat = DensityU.getPrecisionFormat(precision);
    }
