...
public class RippleView extends View {
    ...
    public RippleView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mSpeed = tya.getInt(R.styleable.mRippleView_cSpeed, 2);
        mDensity = tya.getInt(R.styleable.mRippleView_cDensity, 12);
...
//        mPaint.setColor(mColor);
        ...
        RadialGradient rg = new RadialGradient(
                DensityU.dip2px(mContext, 135),
                DensityU.dip2px(mContext, 135),
                DensityU.dip2px(mContext, 120),
                new int[]{Color.parseColor("#00FF414F"), Color.parseColor("#77FF414F"),Color.parseColor("#88FF414F")}
                , new float[]{0,0.9f,1.0f}
                , Shader.TileMode.CLAMP);


        mPaint.setShader(rg);

        mPaint.setStrokeWidth(DensityU.dip2px(mContext, 5));
        if (mIsFill) {
        ...
        mDensity = DensityU.dip2px(mContext, mDensity);
    ...
    private void drawInCircle(Canvas canvas) {
        ...
        if (mRipples.size() > 0) {
            // 控制第二个圆出来的间距
            if (mRipples.get(mRipples.size() - 1).width > DensityU.dip2px(mContext, mDensity)) {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (myWidthSpecMode == MeasureSpec.EXACTLY) {
            ...
            // wrap_content
            mWidth = DensityU.dip2px(mContext, 120);
        }
        ...
        if (myHeightSpecMode == MeasureSpec.EXACTLY) {
            ...
            // wrap_content
            mHeight = DensityU.dip2px(mContext, 120);
        }
